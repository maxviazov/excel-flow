package writer

import (
	"fmt"
	"sort"
	"strconv"
	"time"

	"github.com/xuri/excelize/v2"

	"github.com/maxviazov/excel-flow/internal/pipelines"
)

func WriteMOHFromTemplate(path string, groups map[pipelines.GroupKey]*pipelines.GroupVal) error {
	// Open template
	f, err := excelize.OpenFile("testdata/sample.xlsx")
	if err != nil {
		return fmt.Errorf("failed to open template: %w", err)
	}

	sh := "Sheet1"

	// Sort keys
	keys := make([]pipelines.GroupKey, 0, len(groups))
	for k := range groups {
		keys = append(keys, k)
	}
	sort.Slice(keys, func(i, j int) bool {
		if keys[i].ClientLicense != keys[j].ClientLicense {
			return keys[i].ClientLicense < keys[j].ClientLicense
		}
		if keys[i].OrderID != keys[j].OrderID {
			return keys[i].OrderID < keys[j].OrderID
		}
		return keys[i].Date.Before(keys[j].Date)
	})

	// Write data rows starting from row 2
	row := 2
	for _, k := range keys {
		v := groups[k]
		
		// Copy styles from template row for each column
		for col := 1; col <= 30; col++ {
			templateCell, _ := excelize.CoordinatesToCellName(col, 2)
			targetCell, _ := excelize.CoordinatesToCellName(col, row)
			styleID, _ := f.GetCellStyle(sh, templateCell)
			f.SetCellStyle(sh, targetCell, targetCell, styleID)
		}
		
		// Set values - text for strings, numbers for numeric fields
		f.SetCellStr(sh, fmt.Sprintf("A%d", row), "דולינה גרופ בע\"מ")
		f.SetCellValue(sh, fmt.Sprintf("B%d", row), 511777856)
		f.SetCellStr(sh, fmt.Sprintf("C%d", row), "P1908")
		f.SetCellStr(sh, fmt.Sprintf("D%d", row), time.Now().Format("2006-01-02"))
		// E, F, G are empty (vehicle info)
		if v.ClientName != "" {
			f.SetCellStr(sh, fmt.Sprintf("H%d", row), v.ClientName)
		}
		f.SetCellStr(sh, fmt.Sprintf("I%d", row), "קמעונאי")
		if v.CityCode != "" {
			f.SetCellStr(sh, fmt.Sprintf("J%d", row), v.CityCode)
		}
		if v.Address != "" {
			f.SetCellStr(sh, fmt.Sprintf("K%d", row), v.Address)
		}
		// Convert string to number for L and N
		if licenseNum, err := strconv.ParseFloat(k.ClientLicense, 64); err == nil {
			f.SetCellValue(sh, fmt.Sprintf("L%d", row), licenseNum)
		} else {
			f.SetCellStr(sh, fmt.Sprintf("L%d", row), k.ClientLicense)
		}
		// M is empty (branch number)
		if orderNum, err := strconv.ParseFloat(k.OrderID, 64); err == nil {
			f.SetCellValue(sh, fmt.Sprintf("N%d", row), orderNum)
		} else {
			f.SetCellStr(sh, fmt.Sprintf("N%d", row), k.OrderID)
		}
		// O-V are empty (meat/fish categories)
		f.SetCellValue(sh, fmt.Sprintf("W%d", row), v.TotalWeight)
		// X, Y are empty
		f.SetCellValue(sh, fmt.Sprintf("Z%d", row), v.TotalPackages)
		f.SetCellValue(sh, fmt.Sprintf("AA%d", row), v.TotalWeight)
		f.SetCellValue(sh, fmt.Sprintf("AB%d", row), 1)
		// AC, AD are empty
		
		row++
	}

	return f.SaveAs(path)
}
